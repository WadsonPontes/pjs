...
function converter(nomeOuHTML) { // Converte HTML em Elemento
	...
	if (tipo(nomeOuHTML) == "texto") html = document.querySelector(`#${nomeOuHTML}`);
...
function agendar(atraso, ordem, func, ...args) {
	if (tipo(atraso) != "numero") { // agendar(func, ...args);
		args = [ordem, func, ...args];
		func = atraso;
		ordem = maiorOrdem();
		atraso = 0;
	}
	else if (tipo(ordem) != "numero") { // agendar(atraso, func, ...args);
		args = [func, ...args];
		func = ordem;
		ordem = maiorOrdem();
		
	} // else agendar(atraso, ordem, func, ...args);
	func = (tipo(func) == "texto") ? window[func] : func; // func pode ser String ou Function
...
function repetir(atraso, vezes, func, ...args) {
	if (tipo(atraso) != "numero") { // repetir(func, ...args);
		args = [vezes, func, ...args];
		func = atraso;
		vezes = -1;
		atraso = 0;
	}
	else if (tipo(vezes) != "numero") { // repetir(atraso, func, ...args);
		args = [func, ...args];
		func = vezes;
		vezes = -1;
...
function passou(tempo) {
	window[`tempo_inicial_id_${tempo}`] = window[`tempo_inicial_id_${tempo}`] || performance.now();

	if (performance.now() - window[`tempo_inicial_id_${tempo}`] >= tempo) {
		window[`tempo_inicial_id_${tempo}`] = performance.now();
		return true;
	}
	return false;
}
...
function pedir() {
	...
	let mensagem = "";
	let retorno = tipo(arg[arg.length - 1]) == "funcao" ? true : false;
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			if (retorno) {
				...
				arg[arg.length - 1](JSON.parse(this.responseText));
			}
		}
	};
	if (tipo(arg[1]) == "texto" || tipo(arg[1]) == "numero" || tipo(arg[1]) == "booleano") mensagem = "arg1=" + encodeURIComponent(arg[1]);
...
function tipo(_variavel) {
	switch (typeof _variavel) {
...
function palavra(_palavra) {
  return maiuscula(_palavra) != minuscula(_palavra);
}

function maiuscula(_palavra) {
	return _palavra.toUpperCase();
}

function minuscula(_palavra) {
	return _palavra.toLowerCase();
}
...
class Elemento { // PAI recebe: String, HTML ou Elemento e transforma-se em Elemento
	...
	api = {};
	v = {};
	constructor(tipo, nome, classe, pai, texto, ...args) {
		this.criarHTML(tipo, nome, classe, pai);
		this.nome = nome;
		this.classe = classe;
		this.inicializar(texto, ...args);
	}
	...
	criarHTML(tipo, nome, classe, pai) {
		...
		if (pai != null) this.pai = (tipo(pai) == "texto") ? window[pai] : ((pai.h != null) ? pai : ((this.tipo == "corpo") ? pai : (window[pai.id] || converter(pai))));
		else this.pai = corpo;
	...
	inicializar(texto, ...args) {
		if (texto != null) this.pretexto = texto;

		for (let i = 1; i < args.length; i += 2) {
			if (tipo(this[args[i - 1]]) == "funcao") this[args[i - 1]](args[i]);
			else this[args[i - 1]] = args[i];
		}
	}
	...
	posicao(_x, _y) {
		if (_y == null) {
			this.h.style.left = `${_x.x}px`;
			this.h.style.top = `${_x.y}px`;
		}
		else {
			this.h.style.left = `${_x}px`;
			this.h.style.top = `${_y}px`;
	...
	get tamanho() {
		this.api.tamanho = this.api.tamanho || 100;
		return this.api.tamanho;
	}

	set tamanho(_tamanho) {
		this.api.tamanho = this.api.tamanho || 100;

		if (tipo(_tamanho) == "objeto") {
			this.altura = _tamanho.altura;
			this.largura = _tamanho.largura;
		}
		else {
			_tamanho = inteiro(_tamanho);
			this.x += (this.largura - this.largura * _tamanho / this.api.tamanho) / 2;
			this.y += (this.altura - this.altura * _tamanho / this.api.tamanho) / 2;
			this.altura *= _tamanho / this.api.tamanho;
			this.largura *= _tamanho / this.api.tamanho;
			this.api.tamanho = _tamanho;
		}
	}
	...
	sombra(_horizontal, _vertical, _nivel, _tamanho, _cor, _inverter) {
		if (_horizontal == null) this.h.style.boxShadow = "";
		else if (_vertical == null) this.h.style.boxShadow = `0 0 ${_horizontal}`;
		else if (_nivel == null) this.h.style.boxShadow = `${_horizontal}px ${_vertical}px black`;
		else if (_tamanho == null) this.h.style.boxShadow = `${_horizontal}px ${_vertical}px ${_nivel}`;
		else if (_cor == null) this.h.style.boxShadow = `${_horizontal}px ${_vertical}px ${_nivel}px ${_tamanho}`;
		else if (_inverter == null) this.h.style.boxShadow = `${_horizontal}px ${_vertical}px ${_nivel}px ${_tamanho}px ${_cor}`;
		else this.h.style.boxShadow = `${_horizontal}px ${_vertical}px ${_nivel}px ${_tamanho}px ${_cor} inset`;
		return this;
	}

	addSombra(_horizontal, _vertical, _nivel, _tamanho, _cor, _inverter) {
		if (_vertical == null) this.h.style.boxShadow += `, 0 0 ${_horizontal}`;
		else if (_nivel == null) this.h.style.boxShadow += `, ${_horizontal}px ${_vertical}px black`;
		else if (_tamanho == null) this.h.style.boxShadow += `, ${_horizontal}px ${_vertical}px ${_nivel}`;
		else if (_cor == null) this.h.style.boxShadow += `, ${_horizontal}px ${_vertical}px ${_nivel}px ${_tamanho}`;
		else if (_inverter == null) this.h.style.boxShadow += `, ${_horizontal}px ${_vertical}px ${_nivel}px ${_tamanho}px ${_cor}`;
	...
	evento(_tipo, _func, _func2) {
		...
		else if (_tipo == "beforeunload" || _tipo == "saiu" || _tipo == "saiu da pagina" || _tipo == "saiu_da_pagina" || _tipo == "fechou" || _tipo == "fechou a pagina" || _tipo == "fechou_a_pagina") {
			window.addEventListener("beforeunload", function() { _func(that); });
		}
		return this;
	}
	...
	mover(_duracao, _x, _y, _atraso, _ordem) {
		if (tipo(_x) == "objeto") { // mover(duração, destino);
